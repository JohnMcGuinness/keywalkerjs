define(function() {
	...
	/**
	 * This module is a generic mechanism to enable traversal between element nodes in a group or tree. Covers the most common scenarios for keyboard
	 * navigation as required by compliance with the [WAI-ARIA authoring practices](https://www.w3.org/TR/wai-aria-practices/) for widgets like tabs,
	 * menus, select lists etc.
	 *
	 * Provides the facility to get a target element for:
	 *
	 *   - a group of elements which are traversed in a linear manner such as a group of check boxes, radio buttons or tabs; or
	 *   - a tree of elements defined by a single root node.
	 *
	 * To actually use this library requires a TreeWalker filter to be useful though if one if not supplied then a **very** rudimentary filter function
	 * is available.
	 *
	 * @module
	 */
	return new KeyWalker();
